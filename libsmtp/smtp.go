...
func TLSConnection(serverName string, auth *smtp.Auth) (*smtp.Client, error) {
	// TLS config
...
func SendMailWithAttachments(ssl bool, host string, auth *smtp.Auth, from, subject string, to []string, msg []byte, atch Attachments) error {
	var (
		c   *smtp.Client
		err error
	)
	if ssl {
		c, err = TLSConnection(host, auth)
	} else {
	...
	err = write(
		...
		fmt.Sprintf(`Content-Type: text/html; charset="UTF-8"%s`, CRLF),
	)
	if err != nil {
		return err
	}
